...
int astronave::movimento() { //nel caso si inserisca come input 'a' o 'd', si fa il return(x) poich� ci si muove lungo l'asse delle x, con 'w' e 's' lungo l'asse delle y
	if ((button == 'w') && (y > 0)) {
		y = y - 1;
		return(y);
...
bool astronave::active_raggio_traente() { //se il raggio attraente � disattivato, si attiva, viceversa se � gi� attivato, si disattiva
